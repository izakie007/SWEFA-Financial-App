...
export default function BankTransactions() {
    ...
    const { data: transactions, isLoading } = useQuery({
        queryKey: ['chapter-bank-transactions', profile?.chapter_id],
        queryFn: async () => {
            const { data, error } = await supabase
                .from('chapter_bank_transactions')
                .select('id, amount, transaction_type, transaction_date, reference_number, purposes(name)')
                .eq('chapter_id', profile?.chapter_id)
    ...
    const mutation = useMutation({
        mutationFn: async (values: BankFormValues) => {
            const { error } = await supabase.from('chapter_bank_transactions').insert({
                ...values,
        ...
        onSuccess: () => {
            setSuccess(true);
            reset();
            queryClient.invalidateQueries({ queryKey: ['chapter-bank-transactions'] });
    ...
    return (
        <DashboardLayout navItems={navItems} title="Bank Transactions">
                ...
                <Card className="lg:col-span-1 h-fit">
                    ...
                    `}>
                            ...
                            <Button type="submit" className="w-full" disabled={isSubmitting}>
                                {isSubmitting ? <Loader2 className="animate-spin" size={24} /> : 'Save Transaction'}
                            </Button>
                        </form>
                    </CardContent>
                </Card>
                ...
                <Card className="lg:col-span-2">
                    ...
                    <CardContent className="p-0">
                        <Table>
                            ...
                            <tbody>
                                {isLoading ? (
                                    <TableRow><TableCell colSpan={4} className="p-12 text-center">Loading...</TableCell></TableRow>
                                ) : transactions?.map((t: any) => (
                                    <TableRow key={t.id}>
                                        <TableCell className="text-muted-foreground">{new Date(t.transaction_date).toLocaleDateString()}</TableCell>
                                        <TableCell>
                                            <div className="flex flex-col">
                                                <span className="font-semibold">{t.purposes?.name}</span>
                                                <span className="text-[10px] text-muted-foreground">Ref: {t.reference_number || 'N/A'}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <span className={`flex items-center gap-1 text-[10px] font-bold uppercase ${t.transaction_type === 'DEPOSIT' ? 'text-secondary' : 'text-destructive'}`}>
